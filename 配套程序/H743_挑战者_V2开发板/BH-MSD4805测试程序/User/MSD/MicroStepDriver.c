...
static void TIMx_GPIO_Configuration(void)
{
    ...
    HAL_GPIO_Init(GPIOB,&GPIO_Initure);
	 ...
	 //电机方向输出 GPIO 初始化
    GPIO_Initure.Pin =  MSD_DIR_PIN;
	  GPIO_Initure.Mode = GPIO_MODE_OUTPUT_PP; 
	  GPIO_Initure.Pull  = GPIO_PULLUP;
    GPIO_Initure.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(MSD_DIR_PORT, &GPIO_Initure);
    HAL_GPIO_WritePin(MSD_DIR_PORT,MSD_DIR_PIN,GPIO_PIN_RESET);
    
    //电机使能输出 GPIO 初始化
    GPIO_Initure.Pin =  MSD_ENA_PIN;
    GPIO_Initure.Mode = GPIO_MODE_OUTPUT_PP; 
	  GPIO_Initure.Pull  = GPIO_PULLUP;
    GPIO_Initure.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(MSD_ENA_PORT, &GPIO_Initure);
    HAL_GPIO_WritePin(MSD_ENA_PORT,MSD_ENA_PIN,GPIO_PIN_RESET);
}  
/**
...
void MSD_ENA(FunctionalState NewState)
{
    ...
    {
      //ENA失能，禁止驱动器输出
			HAL_GPIO_WritePin(MSD_ENA_PORT,MSD_ENA_PIN,GPIO_PIN_SET);
      //紧急停止标志位为真
...
static void TIMx_Configuration(void)
{
    ...
    //TIMx的外设寄存器基地址
    TIM_Handle.Instance = MSD_PULSE_TIM;
    //计数模式为向上计数，递增
    TIM_Handle.Init.CounterMode = TIM_COUNTERMODE_UP;
    /* 累计 TIM_Period个后产生一个更新或者中断*/		
    //当定时器从0计数到9999，即为10000次，为一个定时周期
    TIM_Handle.Init.Period = MSD_PULSE_TIM_PERIOD;
    ...
    sConfig.OCPolarity = TIM_OCNPOLARITY_LOW;
		...
		sConfig.Pulse = MSD_PULSE_TIM_PERIOD/2;
		
    HAL_TIM_PWM_ConfigChannel(&TIM_Handle,&sConfig,ADVANCE_TIM_CH);//配置TIM3通道
    HAL_TIM_PWM_Start(&TIM_Handle,ADVANCE_TIM_CH);//开启PWM通道3
   
	 
		HAL_NVIC_SetPriority(MSD_PULSE_TIM_IRQ,1,3);    //设置中断优先级，抢占优先级1，子优先级3
		HAL_NVIC_EnableIRQ(MSD_PULSE_TIM_IRQ);          //开启ITM3中断   
